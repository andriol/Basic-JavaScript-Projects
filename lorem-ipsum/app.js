...
// we could have used the form and pass submit instead of click event
btn.addEventListener("click", function (e) {
  ...
  if (isNaN(value) || value < 0 || value > 9) {
    textLorem.innerHTML = `<p class="result">${text[randomNum]}</p>`;
  } else {
    //iterate over the text where we select an amount of rendom paragraphs to display
    let textSection = text.slice(0, value);
    textSection = textSection.map(function (item) {
      return `<p class="result">${item}</p>`;
    });
    textLorem.innerHTML = textSection.join("");
  }
});
